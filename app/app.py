...
with tab4:
    ...
    本项目是基于 **YOLO (You Only Look Once)** 系列模型的 **火箭检测系统**，用于检测图像中的 **发动机火焰（Engine Flames）**、**火箭本体（Rocket Body）** 等目标。

    ### ✨ 主要功能
    - **多模型兼容**: 无缝支持 YOLOv8, YOLO11, 以及最新的 YOLO26 模型。
    - **高精度实时检测**: 针对火箭检测数据集进行了训练与优化，支持图片和视频的快速检测。
