...
```bash
# 从上次中断的地方继续训练
python train.py --model runs/train/Rocket_Detect/yolo11s_rocket_detect/weights/last.pt --resume
```
...
每个模型的训练结果保存在 `runs/train/{data_name}/{model_name}_rocket_detect/` 目录下：
...
- **Q: 训练结果保存在哪里？**
  - A: 默认保存在 `runs/train/` 目录下。每个实验会根据模型名称生成一个子文件夹，例如 `runs/train/Rocket_Detect/yolo11s_rocket_detect`。
- **Q: 如何查看训练过程的可视化图表？**
...
| `--data`   | `../data/Rocket_Detect/data.yaml` | 数据集配置文件路径             |
| `--epochs` | `100`                             | 训练总轮数                 |
...
本项目使用 Roboflow 开源火箭检测数据集，包含 3 类检测目标：
